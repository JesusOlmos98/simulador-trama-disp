...
export function stringToTuplas(tuplasString: string): [number, number][] {
    try {
        ...
        if (Array.isArray(parsed) && parsed.every(t => Array.isArray(t) && t.length === 2)) {           // Evalua que el array tiene elementos que son tuplas de dos elementos.
            return parsed as [number, number][];                                                        // Asegura que todas las entradas son tuplas [number, number]
        } else { throw new Error("El formato no es válido para tuplas."); }
    } catch (error) {
...
export function stringToList(listString: string): number[] {
    try {
        ...
        } else { throw new Error("El formato no es válido para un array de números."); }
    } catch (error) {
...
export function fechaTiempoToString(fecha: Fecha, tiempo: Tiempo): string {
    const fechaString = `${fecha.dia.toString().padStart(2, "0")}/` +
        `${fecha.mes.toString().padStart(2, "0")}/` +
        `${fecha.anyo.toString().padStart(4, "0")}`;

    const tiempoString = `${tiempo.hora.toString().padStart(2, "0")}h` +
...
// Para los DATETIME, aun sin usar
export function stringToFechaTiempo(fechaTiempo: string): { fecha: Fecha, tiempo: Tiempo } {
    ...
    if (!fechaMatch || !tiempoMatch) {
        ...
        return { fecha: { dia: 1, mes: 1, anyo: 1970 }, tiempo: { hora: 0, min: 0, seg: 0 } };
    }
...
export function fechaToString(fecha: Fecha): string {
    const fechaString = fecha.dia.toString().padStart(2, "0") + "/" + fecha.mes.toString().padStart(2, "0") + "/" + fecha.anyo.toString();
    return fechaString;
...
export function redondeaArribaAbajoFloat(valor: number, decimales: number): number {
    const multiplicador = Math.pow(10, decimales);
    return Math.round(valor * multiplicador) / multiplicador;
}
...
export function uint32ToStringHexUint16(numero: number, maxString: number): string | null {
...
// -------------------------------------------------- hexNumeroToString --------------------------------------------------
export function hexNumeroToString(numero: number, nBytes: number, maxString: number): string | null {
...
// -------------------------------------------------- hexCaracterToByte --------------------------------------------------
export function hexCaracterToByte(primerCaracter: string, segundoCaracter: string): number {
...
export function copiarVariableParaEnvioServer(variable: number, numBytes: number): number {
    switch (numBytes) {
        case 2:
...
export function adaptarVariableRxServer(variable: number, numBytes: number): number {
    return copiarVariableParaEnvioServer(variable, numBytes);
}

// -------------------------------------------------- adaptarVariableMemoriaRxServer --------------------------------------------------
export function adaptarVariableMemoriaRxServer(variable: number, numBytes: number): number {
    return copiarVariableParaEnvioServer(variable, numBytes);
}
